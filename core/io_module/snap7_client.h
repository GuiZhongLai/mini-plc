...
class Snap7ClientBackend;
class Snap7Client : public DevClient
{
public:
    Snap7Client(const std::string &, uint16_t rSize = 0, uint16_t tSize = 0);
    ~Snap7Client();
...
protected:
    Snap7ClientBackend *d;
    friend class Snap7ClientBackend;
};

class Snap7Reply
{
public:
    Snap7Reply(uint16_t size)
        : m_bytes(size), m_datas(new uint8_t[size])
    {
    }
...
class Snap7ClientBackend
{
    friend class Snap7Client;

public:
    Snap7ClientBackend(Snap7Client *parent);
    ~Snap7ClientBackend();
...
private:
    ...
    Snap7Client *pSnap7Client;
    std::thread m_thread;
    bool bQuit = false;

    // snap7 上下文
    TS7Client *ctx = nullptr;
